...
import duyJSON from './duy.json';
import myhanhJSON from './myhanh.json';
// import hoaJSON from './hoa.json';


const volumePath = `myhanh.vti`;
const centerlineJsons = { MyHanh: myhanhJSON, Duy: duyJSON, Aorta: aortaJSON, Spine: spineJSON };
...
function updateDistanceAndDirection() {
  ...
  widgetPlanes[stretchViewType].normal = [0, 0, 1]
  widgetPlanes[stretchViewType].viewUp = [-1, 0, 0];
  ...
  widgetPlanes[crossViewType].normal = [0, 0, 1];
  widgetPlanes[crossViewType].viewUp = [0, -1, 0];
...
function setCenterlineKey(centerlineKey) {
  ...
  console.log("point", centerline.getPoints().getData())
  centerline.modified();
